...
#num of topic
# tf.app.flags.DEFINE_integer('n_topic', 100, 'number of topic feature')
#num of dic
# tf.app.flags.DEFINE_integer('n_topic', 32, 'number of topic feature')
#  num of TF-IDF
tf.app.flags.DEFINE_integer('n_topic', 4000 + 1, 'number of topic feature')
...
#topic feature file
# tf.app.flags.DEFINE_string('topic_file_path', 'topic_feature/topic100.txt', 'topic feature file')
#dic feature file
# tf.app.flags.DEFINE_string('topic_file_path', 'dic_feature/dic32_chi.txt', 'topic feature file')
#  TFIDF feature file
tf.app.flags.DEFINE_string('topic_file_path', 'tra_feature/train_and_test.sample', 'topic feature file')
tf.app.flags.DEFINE_string('test_index', 1, 'test_index')
tf.app.flags.DEFINE_string('embedding_type', 0, 'embedding_type')

class Bi_LSTM_ATT_Topic_9b(object):
    ...
    def __init__(self,
                 ...
                 topic_file_path = FLAGS.topic_file_path,
        ...
        with tf.name_scope('inputs'):
            ...
            self.y = tf.placeholder(tf.float32, [None, self.n_class*2])
            self.doc_len = tf.placeholder(tf.int32, None)
            # input topic
            self.topic = tf.placeholder(tf.float32, [None, self.n_topic])
        ...
        with tf.name_scope('weights'):
            self.weights = {
                ...
                # 'softmax1': init_variable([self.n_hidden*2, 2]),
                # 'softmax2': init_variable([self.n_hidden*2, 2]),
                # 'softmax3': init_variable([self.n_hidden*2, 2]),
                # 'softmax4': init_variable([self.n_hidden*2, 2]),
                # 'softmax5': init_variable([self.n_hidden*2, 2]),
                # 'softmax6': init_variable([self.n_hidden*2, 2]),
                # 'softmax7': init_variable([self.n_hidden*2, 2]),
                # 'softmax8': init_variable([self.n_hidden*2, 2]),
                # 'softmax9': init_variable([self.n_hidden*2, 2]),
                'softmax1': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                'softmax2': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                'softmax3': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                'softmax4': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                'softmax5': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                'softmax6': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                'softmax7': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                'softmax8': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                'softmax9': init_variable([self.n_hidden*2 + self.n_topic, 2]),
                # 'softmax1': init_variable([self.n_topic, 2]),
                # 'softmax2': init_variable([self.n_topic, 2]),
                # 'softmax3': init_variable([self.n_topic, 2]),
                # 'softmax4': init_variable([self.n_topic, 2]),
                # 'softmax5': init_variable([self.n_topic, 2]),
                # 'softmax6': init_variable([self.n_topic, 2]),
                # 'softmax7': init_variable([self.n_topic, 2]),
                # 'softmax8': init_variable([self.n_topic, 2]),
                # 'softmax9': init_variable([self.n_topic, 2]),
            }
        ...
        with tf.name_scope('biases'):
            self.biases = {
                ...
                'softmax1': init_variable([2]),
                'softmax2': init_variable([2]),
                'softmax3': init_variable([2]),
                'softmax4': init_variable([2]),
                'softmax5': init_variable([2]),
                'softmax6': init_variable([2]),
                'softmax7': init_variable([2]),
                'softmax8': init_variable([2]),
                'softmax9': init_variable([2]),
            }

    def model(self, inputs, topic_features):
        ...
        with tf.name_scope('topic_feature'):
            #  add topic features
            outputs = tf.concat([outputs, topic_features], 1)
            # outputs = topic_features
            
        with tf.name_scope('softmax'):
            outputs = tf.nn.dropout(outputs, keep_prob=self.keep_prob)
            predict=[]
            for i in range(1,10):
                tmp = tf.matmul(outputs, self.weights['softmax'+str(i)]) + self.biases['softmax'+str(i)]
                tmp = tf.nn.softmax(tmp)
                predict.append(tmp)

            predict = tf.concat(predict, 1)
    ...
    def run(self):
        ...
        # 
        prob = self.model(inputs, self.topic)
        ...
        with tf.name_scope('readData'):
            ...
            tr_x, tr_y, tr_doc_len, tr_topic, te_x, te_y, te_doc_len, te_topic = load_data(
        ...
        with tf.Session() as sess:
            ...
            for i in xrange(self.training_iter):
                ...
                for train, _ in self.get_batch_data(tr_x, tr_y, tr_doc_len, tr_topic, self.batch_size, self.Keep_Prob, test=False):
    ...
    def get_batch_data(self, x, y, doc_len, topic, batch_size, keep_prob, test= False):
