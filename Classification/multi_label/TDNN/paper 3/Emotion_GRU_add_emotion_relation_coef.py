...
from PrepareData import batch_index, load_w2v, load_data_for_Emotion_CNN1
...
tf.app.flags.DEFINE_float('rela_reg', 0.0000001, 'relation regularization')
...
tf.app.flags.DEFINE_string('train_file_path1', 'renCECps_multi_class_probability.txt', 'training file p')
...
class Emotion_CNN(object):
    ...
    def __init__(self,
                ...
                #
                 ...
                 train_file_path1=FLAGS.train_file_path1,
        ...
        self.train_file_path1 = train_file_path1
        self.w2v_file = w2v_file
        self.test_index = test_index
        self.embedding_type = embedding_type
        self.scope=scope

        self.omiga = np.zeros([self.n_class, self.n_class])
...
#  
        ...
        with tf.name_scope('sentence_attention'):
                    ...
                    # alpha = tf.nn.softmax(tf.reshape(tf.matmul(u, self.weights['u_2']), [batch_size, 1, self.max_doc_len]))  # batch_size * 1 * n_step
                    ...
                    outputs = tf.reshape(tf.matmul(alpha, outputs), [-1,self.n_hidden])  # batch_size * 1 * n_hidden   V batch_size  * n_hidden
...
#
    ...
    def run(self):
        ...
        with tf.name_scope('loss'):
            ...
            for i in range(8):
                for j in range(8):
                    relation += tf.nn.l2_loss(tmp[:,i] - tmp[:,j]) * self.omiga[i,j]
        ...
        with tf.name_scope('readData'):
            ...
            tr_x, tr_y, tr_doc_len, te_x, te_y, te_doc_len, ev_x, ev_y, ev_doc_len, self.omiga= load_data_for_Emotion_CNN1(
                self.train_file_path,
                self.train_file_path1,
