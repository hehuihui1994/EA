...
# file path
tf.app.flags.DEFINE_string('train_file_path', '../resources/Emo_Cause_corpus_HHH_simple_whole_distance.txt', 'training file')
...
class BiLSTM_ATT(object):
    ...
    def run(self):
        ...
        prob_sen = self.model(inputs, self.topic)
        ...
        with tf.name_scope('readData'):
            ...
            tr_x, tr_y, tr_y_sen, tr_sen_len, tr_doc_len, tr_topic, te_x, te_y, te_y_sen, te_sen_len, te_doc_len, te_topic= load_data(
        ...
        with tf.Session(config=tf_config) as sess:
            ...
            for i in xrange(self.training_iter):
                ...
                for train, _ in self.get_batch_data(tr_x, tr_y, tr_y_sen, tr_sen_len, tr_doc_len, tr_topic, self.batch_size, FLAGS.keep_prob1, FLAGS.keep_prob2, test=False):
    ...
    #获取batch数据
    def get_batch_data(self, x, y, y_sen, sen_len, doc_len, topic, batch_size, keep_prob1, keep_prob2, test= False):
