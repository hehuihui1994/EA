...
class EATT_BiLSTM_dis0_emo(object):
       ...
       #所有的模型可训练参数
        with tf.name_scope('weights'):
            self.weights = {
                ...
                'w_1': tf.Variable(tf.random_uniform([4 * self.n_hidden + self.embedding_dim, 4 * self.n_hidden + self.embedding_dim], -0.01, 0.01)),
                'u_1': tf.Variable(tf.random_uniform([4* self.n_hidden + self.embedding_dim, 1], -0.01, 0.01)),                
                # Attention doc
                ...
                'softmax_sen': init_variable([2 * self.n_hidden, self.n_sentence_class]),                
                # softmax doc
        ...
        with tf.name_scope('biases'):
            self.biases = {
                ...
                'w_1': tf.Variable(tf.random_uniform([4 * self.n_hidden + self.embedding_dim], -0.01, 0.01)),                
    ...
    def model(self, inputs, aspect_id):
        ...
        with tf.name_scope('word_attention'):
            ...
            hiddens = outputs
            # V_E
            V_E = tf.concat([hiddens, aspect_embedding], 2)

            # e_w^E
            ewE = inputs

            # V_E与e_w^E逐元素相乘，不改变shape
            new_aspect_embedding = tf.multiply(ewE, V_E)

            h_t = tf.reshape(tf.concat([hiddens, new_aspect_embedding], 2), [-1, 4 * self.n_hidden + self.embedding_dim])
