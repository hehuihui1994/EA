...
tf.app.flags.DEFINE_float('keep_prob2', 0.7, 'softmax layer dropout keep prob')
...
class Mem_LSTM_dis0_emo(object):
    ...
    def model(self, inputs, aspect_id):
        ...
        # 不需要再将aspect_embedding复制多份，然后与隐藏层拼接
        ...
        # with tf.name_scope('word_attention'):
        #     # hiddens = tf.reshape(outputs, [-1, self.max_sentence_len, 2*self.n_hidden])
        #     hiddens = outputs
        #     h_t = tf.reshape(tf.concat([hiddens, aspect_embedding], 2), [-1, 2 * self.n_hidden + self.embedding_dim])

        #     # h_t = tf.concat([hiddens, aspect_embedding], 2)
        #     # h_t = tf.reshape(h_t, [-1, 2 * self.n_hidden + self.embedding_dim])
        ...
        with tf.name_scope('word_attention'):
            ...
            # [-1,1,200]
            # aspect_embedding转置
            aspect_embedding = tf.transpose(aspect_embedding, [0, 2, 1])
            m = tf.matmul(h, aspect_embedding)
            alpha = self.softmax(m, self.sen_len, self.max_sentence_len)

            # alpha [-1, 50, 1]
            # h [-1, 50, 200]
            # 需要把alpha转置
            alpha = tf.transpose(alpha, [0, 2, 1])
            outputs = tf.matmul(alpha, h)

            # [-1, 1, 200]
            # outputs = tf.add(outputs, aspect_embedding)
            outputs = tf.reshape(outputs, [batch_size, self.n_hidden])
            aspect_embedding = tf.reshape(outputs, [batch_size, self.n_hidden])
            outputs = tf.add(outputs, aspect_embedding)
